...
> [ASSUMPTION — PROPOSAL] The market category below reflects a planning hypothesis based on Bridge Translation as the primary known capability. As Bridge's broader companion scope (Appointment, Hearing, Independence, Legacy) is defined, the market definition will need revision. This is not the final market definition and requires founder approval before use in any external communication or financial planning.

```
Category: Communication companions for people facing barriers to connection and understanding

Note on scope: This market category should be revisited as Bridge's companion modules are 
defined. The translation-focused market definition below was drafted before the broader 
companion scope was established and may not reflect the full market Bridge addresses.

Bridge Translation: Personal communication companions for multilingual relationships — 
competing not in the general translation market but in the narrower category defined by: 
personal relationships + language gap + companion-model product design.
```
